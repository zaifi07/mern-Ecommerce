...
const productSlice = createSlice({
    ...
    reducers: {
        ...
        setBrandFilter(state, action) {
            ...
            state.filters.brand = action.payload; // [] for clear, [id] for filter
        },
        setCategoryFilter(state, action) {
            state.filters = state.filters || {};
            // If action.payload is an array, use it. If it's a string, wrap in array (unless empty).
        ...
        setPriceFilter(state, action) {
            state.filters = state.filters || {};
            state.filters.price = action.payload; // { max: 3000 }
        },
...
export const selectBrandFilter = (state) => state.ProductSlice.filters?.brand;
export const selectCategoryFilter = (state) => state.ProductSlice.filters?.category;
export const selectPriceFilter = (state) => state.ProductSlice.filters?.price;
...
// Inside fetchProductsAsync or similar function:
const buildQueryParams = (filters) => {
    const params = new URLSearchParams();
    if (filters.brand && Array.isArray(filters.brand)) {
        filters.brand.filter(Boolean).forEach(b => params.append('brand', b));
    }
    if (filters.category && Array.isArray(filters.category)) {
        filters.category.filter(Boolean).forEach(c => params.append('category', c));
    }
    // ...other params...
    return params;
};

// Then use buildQueryParams(filters) to create your query string
